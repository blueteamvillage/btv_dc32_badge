  ...
  for (int i = 0; i < 255; i++)
  {
    ...
    if (pos < 85)
    {
      ...
      if (pos == 0)
      {
        ...
        if (MyRandom == 1)
        {
          testFilledCircles(10, GC9A01A_BLUE);
        }
        if (MyRandom == 2)
        {
...
unsigned long testTCHvalueText()
{
  ...
  tft.print("  TCH01 ");
  tft.setTextColor(GC9A01A_WHITE);
  tft.print(Touch01_Value);
  tft.setTextColor(GC9A01A_BLUE);
  tft.print("/");
  tft.println(Touch01_Threshold);
  tft.print("  TCH03 ");
  tft.setTextColor(GC9A01A_WHITE);
  tft.print(Touch03_Value);
  tft.setTextColor(GC9A01A_BLUE);
  tft.print("/");
  tft.println(Touch03_Threshold);
  tft.print("  TCH05 ");
  tft.setTextColor(GC9A01A_WHITE);
  tft.print(Touch05_Value);
  tft.setTextColor(GC9A01A_BLUE);
  tft.print("/");
  tft.println(Touch05_Threshold);
  return micros() - start;
}
//
